...
const Command: Moderation.ICommand = {
    usages: ['erkek', 'man', 'e'],
    description: 'Belirttiğiniz kullanıcıyı erkek olarak kayıt edersiniz.',
    examples: ['e @kullanıcı isim yaş', 'e 123456789123456789 isim yaş'],
    ...
    execute: async ({ client, message, args, guildData }) => {
        ...
        const roles: string[] = (guildData.manRoles || []).filter((r) => message.guild.roles.cache.has(r));
        ...
        if (registerLogChannel) {
            registerLogChannel.send({
                content: `${member} (${inlineCode(member.id.toString())}) adlı kullanıcı ${
                    message.author
                } (${inlineCode(message.author.username)}) tarafından kayıt edildi.`,
            });
        }
        ...
        message.channel.send({
            embeds: [
                new EmbedBuilder({
                    description: `${member} üyesi başarıyla ${inlineCode('erkek')} olarak kayıt edildi.`,
                }),
            ],
        });
        ...
        await UserStatModel.updateOne(
            { id: message.author.id, guild: message.guildId },
            {
                $push: {
                    registers: {
                        type: guildData.manRoles?.some((r) => message.guild.roles.cache.has(r)) ? RegisterFlags.Man : RegisterFlags.Normal,
                        user: member.id,
                        time: Date.now(),
                    }
                },
